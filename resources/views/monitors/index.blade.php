...
font-weight: normal!important;
    ...
    <body>
        ...
        if (\App\Models\Competition::where('status', 1)->count() == 0) {
            ...
            $comp = \App\Models\Competition::select(['id', 'name_' . $lang . ' as title', 'description_' . $lang . ' as description'
                        , 'logo', 'active_class', 'active_phase as phase' ,'active_section' ,'champion_calc_type'])->where('status', 1)->orderBy('id', 'desc')->first();
 ...
 <hr /> 
    ...
    foreach (\App\Models\CompetitionGroup::where('start_dob', '>=', $c->start_dob)
        ...
        ?>
                            ...
                            <?php
                                    ...
                                    <td class="horse-chmp-status-{{$reg->present_in_championship}}">{{$reg->reg_number}}</td>
                                                <?php
    ...
    min-width: 12rem;">{{$judge->name()}}</td>
                                ...
                                <tr <?=$comp->champion_calc_type == 'points-based' ?  'style="display: none"' : "" ?>>
